...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        console.log(data);
        const html = data.active_users.map(user => {return `<li>${user}</li>`}).join('\n')
        qs('#users-list').innerHTML = `<ul>${html}</ul>
        <span id="update" aria-live="polite" aria-atomic="false" aria-relevant="text additions">
        ${data.user_joined} has joined the chat
        </span>`
        ...
        qs('#users-list').innerHTML = `<ul>${html}</ul>
        <span id="update" aria-live="polite" aria-atomic="false" aria-relevant="text additions">
        ${data.user_left} has left the chat
        </span>`
    } else if (data.type === "chat") {
        console.log('A user has just sent a chat message:');
        console.log(data);
        const html = `<div class=${data.username == username ? 'right' : 'left'}>
                        <strong>${data.username}:</strong> ${data.text}
                     </div>`
        qs('#chat').innerHTML += html
    } else {
